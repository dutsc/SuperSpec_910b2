...
def draft_sample(gpu_id, task_queue, result_queue, config):
    ...
    if "Qwen" in llm_model_path:
        model = Qwen2ForCausalLM.from_pretrained(
            config["ssm"]["model_device_map"][gpu_id],
    ...
    while True:
        ...
        if clear_cache:
            if model._past_key_values is not None:
                del model._past_key_values
            if model._logits_history is not None:
                del model._logits_history
        ...
        for j in range(lookahead):
            ...
            start_draft = time.perf_counter_ns() / 1e6
            draft_outputs = model.generate(fin_prompt_seq, 1)
            end_draft = time.perf_counter_ns() / 1e6
            draft_latency = end_draft - start_draft
            if config["timeline"]:
                print(f"{draft_latency = } ms.")
...
def distribute_data(prompt, ssm_queue, draft_queue, event, clear_cache, config):
    ...
    for i in range(len(SSM_GPUs)):
        ...
        gpu_outputs[task_id].extend(pair_list)  # 对应位置堆放
