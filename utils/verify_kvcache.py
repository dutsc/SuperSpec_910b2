...
class KVCacheModel:
    ...
    def _forward_with_kvcache(
        ...
        **model_kwargs,
        ...
        if self._past_key_values is None:
            ...
            # self._new_key_values = (
            #     outputs.new_key_values
            # )  # if auto then delete this row
    ...
    def rollback_with_mask(self, mask_seq):
        ...
        # print(f'{self._past_key_values[14][0].device = }')
        # print(f'{self._past_key_values[24][0].device = }')
        # 如果需要，也应相应调整self._logits_history
    ...
    def rollback_with_batch_winner(self, end_pos, batch_winner_index):
        ...
        # print(f"{end_pos = },{self._batch_first = }")
        past_key_values_trimmed = []
        assert self._new_key_values
        if self._batch_first:
            for idx, kv in enumerate(self._new_key_values):  # 遍历n_layers
                # print(f'{self._new_key_values[0][0].shape = }')
                # print(f'{self._new_key_values[0][1].shape = }')
                # print(f'{type(self._new_key_values) = }')
                # print(f'{len(self._new_key_values) = }')
                k, v = kv
            ...
            # self._batch_kvcache_buffer = past_key_values_trimmed
            self._batch_first = False
            self._past_key_values = past_key_values_trimmed

        else:
            # print(f'k.shape:{len(self._past_key_values[0][0][0][0]) = }')
            # print(f'new_k.shape:{len(self._batch_kvcache_buffer[0][0][0][0]) = }')
            # new_token_len = len(self._batch_kvcache_buffer[0][0][0][0]) - len(self._past_key_values[0][0][0][0])

            # print(f'k:{self._past_key_values[0][0][0][0][9][0] = }')
            # print(f'k:{self._past_key_values[0][0][0][0][-2][0] = }')
            # print(f'new_k:{self._batch_kvcache_buffer[0][0][0][0][9][0] = }')
            # print(f'new_k:{self._batch_kvcache_buffer[0][0][0][0][-11][0] = }')  # -9
            # print(f'k:{self._past_key_values[0][0][0][0][10][0] = }')
            # print(f'k:{self._past_key_values[0][0][0][0][-1][0] = }')
            # print(f'new_k:{self._batch_kvcache_buffer[0][0][0][0][10][0] = }')
            # print(f'new_k:{self._batch_kvcache_buffer[0][0][0][0][-10][0] = }')
            # new_k的seq_len维度比k大 9
            for idx, kv in enumerate(self._new_key_values):
                k, v = kv
                # print(f'{k.shape = }')
                # print(f'{v.shape = }')
                ...
                # print(f'{past_k.device = }')
                # print(f'{k.device = }')
                # new_k, new_v = self._batch_kvcache_buffer[idx]
                # new_k = new_k[:, :, -new_token_len:, :]
                # new_v = new_v[:, :, -new_token_len:, :]
