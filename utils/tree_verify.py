...
def compute_and_tree_verify(gpu_outputs, llm, prompt, config):
    ...
    mask_init = 1 - torch.triu(
        torch.ones(mask.shape[0], mask_N, mask_N, dtype=float), diagonal=1
    ).to(prompt.device)
...
def verify_greedy(root, llm_logits):
    ...
    while len(H) > 0:
        ...
        llm_token = torch.multinomial(
            llm_logits[:, index_mapping[chosen_child.parent] + 1, :], num_samples=1
        )
    ...
    if all_accepted:
        append_new_token = torch.multinomial(
            llm_logits[:, index_mapping[chosen_child] + 1, :], num_samples=1
        )
        V.append(append_new_token)
    else:
        resample_token = torch.multinomial(
            llm_logits[:, index_mapping[chosen_child.parent] + 1, :], num_samples=1
        )
