...
#     # 获取所有子列表中的最大长度
#     max_length = max(len(outputs) for outputs in gpu_outputs)
...
#         # 填充 tokens 和 logits
#         if len(t_tokens) < max_length:
#             last_token = t_tokens[-1] if t_tokens else None  # 获取最后一个 token
#             t_tokens.extend([last_token] * (max_length - len(t_tokens)))
        
#         if len(t_logits) < max_length:
#             last_logits = t_logits[-1] if t_logits else None  # 获取最后一个 logits
#             t_logits.extend([last_logits] * (max_length - len(t_logits)))

#         # 将 logits 转移到指定的 GPU 上
#         t_logits = torch.stack(t_logits, dim=0).to(f'cuda:{LLM_GPU}', non_blocking=True)
