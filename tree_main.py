...
from utils.tree_verify import compute_and_tree_verify
...
original_text = (
    "Could you please give me some advice on programming?"  # "Hello, my name is"
)
...
if __name__ == "__main__":
    ...
    # llm_model_path = "meta-llama/Llama-3.1-8B"

    if "Qwen" in llm_model_path:
        init_llm = Qwen2ForCausalLM.from_pretrained(
            llm_model_path, torch_dtype=torch.float16, device_map="cuda:0"   #应该是"cuda:"+ str(LLM_GPU)
        )
    elif "opt" in llm_model_path:
        init_llm = OPTForCausalLM.from_pretrained(
            llm_model_path, torch_dtype=torch.float16, device_map="cuda:0"
        )
    ...
    for i in range(10):
        ...
        new_tokens, accept_num, root, tree_infer_time = compute_and_tree_verify(
            gpu_outputs, llm, prompt, config
        )
    ...
    for prompt in tqdm(sentences, desc="Processing sentences"):
        ...
        while True:
            ...
            single_prompt_infer_times.append(tree_infer_time)
            mask_seq = create_mask_seq(root)
            # root.print_tree()
            # root.print_mask()
            # print(f'mask_seq: {mask_seq}')
            llm.rollback_with_mask(mask_seq)
    ...
    for i, question in enumerate(sentences):
        question_dict = {
            ...
            "mean_verify_time": dataset_infer_times[i],
            "rounds": spec_num,
    ...
    # 打印JSON数据
    with open("./outputs/opt/tree_output_" + datasets[dataset_index], "w") as json_file:
