        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            if bsz == 1:
                ...
                cat_value_states = torch.cat(
                    [v.repeat(bsz, 1, 1, 1), value_states], dim=2
                )
        ...
        attn_weights = torch.matmul(
            query_states, cat_key_states.transpose(2, 3)
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(
                v
                for v in [hidden_states, present_cache, new_cache, all_hidden_states]
                if v is not None
            )
