...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def forward(
        ...
        kv_seq_len = key_states.shape[-2] # 新的kvcache的长度 K+1=9
        # print(f'{kv_seq_len = }')
        
        # print(f'{self.layer_idx = }')
        if past_key_value is not None:
            kv_seq_len += past_key_value.get_usable_length(kv_seq_len, self.layer_idx) # 总的kvcache的长度
            # print(f'total:{kv_seq_len = }')
            
        cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids) # 新的kvcache长度 9
        # print(f'before update:{key_states.shape = }')
        
        if past_key_value is not None:
            cache_kwargs = {"sin": sin, "cos": cos}  # Specific to RoPE models
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
        
        # print(f'after  update:{key_states.shape = }')
