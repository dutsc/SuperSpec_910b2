...
class OPTAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            if bsz == 1:
                #print(f'bsz=1 item1.shape = {past_key_value[0].shape}')
                #print(f'bsz=1 item2.shape = {key_states.shape}')
        ...
        value_states = value_states.view(*proj_shape) #(bsz*num_heads,seq_len,head_dim)
        ...
        # if bsz==3:
        #     s2 = time.perf_counter_ns() / 1e6
        #     key_states_list=[]
        #     value_states_list=[]
        #     attn_weights_list=[]
        #     query_states_list=[]
        #     for bsz_index in range(bsz):
        #         #print(f'bsz=3 item1.shape = {past_key_value[0].shape}')
        #         #print(f'bsz=3 item2.shape = {key_states[bsz_index,:,:,:].unsqueeze(0).shape}')
        #         # print(f'{past_key_value[0].shape = }')
        #         key_states_list.append(torch.cat([past_key_value[0], key_states[bsz_index,:,:,:].unsqueeze(0)], dim=2))
        #         value_states_list.append(torch.cat([past_key_value[1], value_states[bsz_index,:,:,:].unsqueeze(0)], dim=2))
        #         t_shape=(self.num_heads,-1,self.head_dim)
        #         src_len = key_states_list[0].view(*t_shape).size(1)
             
        #         # print(f'bmm1.shape = {query_states[bsz_index*self.num_heads:(bsz_index+1)*self.num_heads,:,:].shape}')
        #         # print(f'bmm2.shape = {key_states_list[bsz_index].view(*t_shape).transpose(1, 2).shape}')
                
        #         attn_weights_list.append(torch.bmm(query_states[bsz_index*self.num_heads:(bsz_index+1)*self.num_heads,:,:],
        #          key_states_list[bsz_index].view(*t_shape).transpose(1, 2)))
             
                 
        #     attn_weights=torch.cat(attn_weights_list,dim=0)
        #     past_key_values=(torch.cat(key_states_list,dim=0),torch.cat(value_states_list,dim=0))
        #     #print(f'bsz=3 attn_weights.shape = {attn_weights.shape}')
            
        #     e2 = time.perf_counter_ns() / 1e6
        #     bsz3 = e2 - s2
        #     print(f'{bsz3 = }')
        ...
        # if bsz==3:
        #     attn_output_list=[]
        #     for bsz_index in range(3):
        #         #print(f'value.shape = {value_states_list[bsz_index].shape}')
        #         attn_output_list.append(torch.bmm(attn_probs[bsz_index*self.num_heads:(bsz_index+1)*self.num_heads,:,:],
        #         value_states_list[bsz_index].view(*t_shape)
        #         ))
        #     attn_output=torch.cat(attn_output_list,dim=0)
